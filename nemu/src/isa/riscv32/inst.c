...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //I* Type
  INSTPAT("0000000 ????? ????? 001 ????? 00100 11", srai   , I, R(rd) = src1>>imm);
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srli   , I, R(rd) = src1>>imm);
  INSTPAT("0000000 ????? ????? 100 ????? 00100 11", slli    , I, R(rd) = src1<<imm);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 110 0011", blt     , B, s->dnpc=(src1<src2)?s->pc+imm:s->pc+4);
  ...
  INSTPAT_END();
  printf("pc: %x a0: %x a1: %x a4: %x  a5 %x s0: %x, s1:%x s3:%x \n",s->pc,R(10),R(11),R(14),R(15),R(8),R(9),R(19));
