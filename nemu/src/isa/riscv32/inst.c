...
enum {
  TYPE_I, TYPE_R,TYPE_J, TYPE_U, TYPE_S, TYPE_B,
  TYPE_N, // none
};
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //I* Type
  INSTPAT("000 0000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = src1>>imm);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 110 0011", bge     , B, s->dnpc=(src1>=(int)src2)?s->pc+imm:s->pc+4);
  INSTPAT("??????? ????? ????? 111 ????? 110 0011", bgeu    , B, s->dnpc=(src1>=(unsigned)src2)?s->pc+imm:s->pc+4);
  INSTPAT("??????? ????? ????? 100 ????? 110 0011", blt     , B, s->dnpc=(src1<(int)src2)?s->pc+imm:s->pc+4);
  INSTPAT("??????? ????? ????? 110 ????? 110 0011", bltu    , B, s->dnpc=(src1<(unsigned)src2)?s->pc+imm:s->pc+4);
  ...
  INSTPAT_END();
  printf("pc: %x a0: %x s1 %x s2: %x s5: %x ra: %x\n",s->pc,R(10),R(9),R(18),R(21),R(1));
