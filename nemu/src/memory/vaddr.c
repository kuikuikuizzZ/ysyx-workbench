...
word_t vaddr_ifetch(vaddr_t addr, int len) {
  paddr_t paddr = isa_mmu_translate(addr, len, MEM_READ);
...
word_t vaddr_read(vaddr_t addr, int len) {
  ...
  mem_access_hook(MEM_READ,paddr,data);
  // printf("vaddr_read addr: " FMT_WORD " len: %d data %x\n", addr, len,data);
  return data;
}

void vaddr_write(vaddr_t addr, int len, word_t data) {
  paddr_t paddr = isa_mmu_translate(addr, len, MEM_WRITE);
  // printf("vaddr_write addr: " FMT_WORD " len: %d data %x\n", addr, len,data);
  mem_access_hook(MEM_WRITE,paddr, data);
