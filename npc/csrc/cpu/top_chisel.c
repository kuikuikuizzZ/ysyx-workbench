...
uint32_t top_gpr(int i) {
    ...
    uint32_t gpr_i ;
    IFDEF(CONFIG_SOC,gpr_i=(uint32_t)_rootp->ysyxSoCFull__DOT__asic__DOT__cpu__DOT__cpu__DOT__reg_file__DOT__ysyx_24100012_regfile_mem_ext__DOT__Memory[i];);
...
uint32_t top_alu_out() {
    if (!_rootp) return 0;
    uint32_t alu_out;
    IFDEF(CONFIG_SOC,alu_out=(uint32_t)_rootp->ysyxSoCFull__DOT__asic__DOT__cpu__DOT__cpu__DOT__lsu_io_exe_mem_bits_ralu_out);
    return alu_out;
}
