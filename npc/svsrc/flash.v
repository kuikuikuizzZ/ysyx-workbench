...
module flash (
  ...
  localparam data_t  = 3'b010;
  localparam err_t   = 3'b011;
  
  
  wire reset = ss;  

  // typedef enum [2:0] {  } state_t;
  reg [2:0]  state;
  reg [7:0]  counter;
  reg [7:0]  cmd;
  reg [23:0] addr;
  ...
  wire [31:0] data_tmp = counter == 8'd0 ? data_bswap : data;
  always@(posedge sck or posedge reset) begin
    if (reset) data <= 32'd0;
    else if (state == data_t) begin
      data <= { data_tmp[30:0], 1'b0 };
    end
  end
  wire [31:0] data_tmp2 = (state == data_t && counter == 8'd0) ? data_bswap : data;
  assign miso = ss ? 1'b1 : data_tmp2[31];
