...
class AXI4MasterTest extends AnyFlatSpec with Matchers {
  
  // 测试配置
  
  // 状态监控函数
  def monitorState(dut: AXI4Master): String = {
    val rstate = dut.io.debug.rstate.peekValue().asBigInt.toInt
    val wstate = dut.io.debug.wstate.peekValue().asBigInt.toInt
  ...
  "AXI4Master" should "handle single read operation correctly" in {
    implicit val conf = Config()      
    simulate(new AXI4Master()) { dut =>
      ...
      // 设置读请求
      dut.io.req.bits.raddr.poke(0x1000.U)
      dut.io.req.bits.ren.poke(true.B)
      dut.io.req.bits.wen.poke(false.B)
      dut.io.req.bits.typ.poke(2.U) // 32位访问
      dut.io.req.valid.poke(true.B)
      // 验证AR通道信号
      dut.io.axi_io.ar.valid.expect(true.B)
      dut.io.axi_io.ar.bits.addr.expect(0x1000.U)
      println(s"Cycle 1 - State: ${monitorState(dut)}")
      ...
      // 从设备返回数据
      dut.io.axi_io.r.valid.poke(true.B)
      dut.io.axi_io.r.bits.data.poke(0x12345678.U)
      dut.io.axi_io.r.bits.resp.poke(0.U) // OKAY响应
      dut.io.axi_io.r.bits.last.poke(true.B)
  ...
  "AXI4Master" should "handle single read wait rvalid correctly" in {
    implicit val conf = Config()      
    simulate(new AXI4Master()) { dut =>
  ...
  "AXI4Master" should "handle single write correctly" in {
    implicit val conf = Config()      
    simulate(new AXI4Master()) { dut =>
      ...
      // 立即执行写操作
      dut.io.req.bits.waddr.poke(0x7000.U)
      dut.io.req.bits.data.poke(0x12345678.U)
      dut.io.req.bits.mask.poke("b0001".U)
      dut.io.req.bits.typ.poke(3.U)
      dut.io.req.bits.wen.poke(true.B)
      dut.io.req.bits.ren.poke(false.B)
      dut.io.req.valid.poke(true.B)
      ...
      dut.io.debug.is_write.expect(true.B)
      dut.io.debug.wstate.expect(2.U) // ws_wait_ready
      dut.io.axi_io.w.bits.data.expect(0x12345678.U)
      dut.io.axi_io.aw.bits.size.expect(2.U)
      dut.io.axi_io.w.bits.strb.expect("b0001".U)

      dut.io.axi_io.b.valid.poke(true.B)
      dut.io.axi_io.b.bits.resp.poke(0.U)
  ...
  "AXI4Master" should "handle multi-cycle read with ARREADY delay" in {
    implicit val conf = Config()       
    simulate(new AXI4Master()) { dut =>
      ...
      // 设置读请求
      dut.io.req.bits.raddr.poke(0x2000.U)
      dut.io.req.bits.ren.poke(true.B)
      dut.io.req.bits.wen.poke(false.B)
      dut.io.req.bits.burstlen.poke(3.U) // 4次传输
      dut.io.req.bits.burst.poke(1.U) // INCR模式
      dut.io.req.bits.typ.poke(3.U)
      dut.io.req.valid.poke(true.B)
      ...
      for (i <- 0 until 4) {
        dut.io.axi_io.r.valid.poke(true.B)
        dut.io.axi_io.r.bits.data.poke((0x1000 + i * 4).U)
        dut.io.axi_io.r.bits.last.poke(i == 3)
  ...
  "AXI4Master" should "handle back-to-back read and write operations" in {
    implicit val conf = Config()       
    simulate(new AXI4Master()) { dut =>
      ...
      // 先执行读操作
      dut.io.req.bits.raddr.poke(0x6000.U)
      dut.io.req.bits.ren.poke(true.B)
      dut.io.req.bits.typ.poke(2.U)
      dut.io.req.bits.wen.poke(false.B)
      dut.io.req.valid.poke(true.B)
      ...
      dut.clock.step(2)
      dut.io.req.bits.ren.poke(false.B)
      dut.io.axi_io.ar.bits.size.expect(1.U)

      dut.io.axi_io.r.valid.poke(true.B)
      dut.io.axi_io.r.bits.data.poke(0xAAA5555.U)
      dut.io.axi_io.r.bits.last.poke(true.B)
      dut.io.axi_io.r.bits.resp.poke(0.U)
      ...
      dut.io.req.bits.waddr.poke(0x7000.U)
      dut.io.req.bits.data.poke(0x12345678.U)
      dut.io.req.bits.mask.poke("b1111".U)
      dut.io.req.bits.typ.poke(1.U)
      dut.io.req.bits.wen.poke(true.B)
      dut.io.req.bits.ren.poke(false.B)
      dut.io.req.valid.poke(true.B)
      ...
      dut.io.axi_io.w.ready.poke(true.B)
      dut.clock.step(2)
      dut.io.axi_io.aw.bits.size.expect(0.U)
      dut.io.axi_io.w.bits.strb.expect("b1111".U)
  ...
  "AXI4Master" should "handle error responses correctly" in {
    implicit val conf = Config()       
    simulate(new AXI4Master()) { dut =>
      ...
      // 测试读错误
      dut.io.req.bits.raddr.poke(0x8000.U)
      dut.io.req.bits.ren.poke(true.B)
      dut.io.req.valid.poke(true.B)
      ...
      dut.io.debug.rstate.expect(2.U) // rs_wait_arready
      dut.io.axi_io.r.valid.poke(true.B)
      dut.io.axi_io.r.bits.data.poke(0.U)
      dut.io.axi_io.r.bits.resp.poke(2.U) // SLVERR错误
      dut.io.axi_io.r.bits.last.poke(true.B)
      ...
      // 测试写错误
      dut.io.req.bits.waddr.poke(0x9000.U)
      dut.io.req.bits.data.poke(0xFFFFFFF.U)
      dut.io.req.bits.wen.poke(true.B)
      dut.io.req.bits.ren.poke(false.B)
      ...
      dut.io.axi_io.w.ready.poke(true.B)
      dut.io.axi_io.b.valid.poke(true.B)
      dut.io.axi_io.b.bits.resp.poke(3.U) // DECERR错误
...
// 运行所有测试的入口对象
object AXI4MasterTestRunner {
  def main(args: Array[String]): Unit = {
    // 这里可以添加具体的测试运行逻辑
    println("AXI4Master 测试套件")
  }
}