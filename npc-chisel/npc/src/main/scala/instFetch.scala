...
class ysyx_24100012_InstFetch(implicit conf: ysyx_24100012_Config) extends Module {
  ...
  // Pipeline Interface
  val if_inst = Mux(cache.io.valid,cache.io.inst,RegEnable(cache.io.inst,BUBBLE,io.ifu_dec.ready ))
  val if_valid = Mux(cache.io.valid,cache.io.valid,RegEnable(cache.io.valid,io.ifu_dec.ready))
