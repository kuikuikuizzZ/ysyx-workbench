...
class InstFetch(implicit conf: Config) extends Module {
  ...
  val if_valid = Mux(should_kill,true.B,Mux(cache.io.valid,cache.io.valid,RegEnable(cache.io.valid && !io.ifu_dec.ready,cache.io.valid || io.ifu_dec.ready || should_kill)))
