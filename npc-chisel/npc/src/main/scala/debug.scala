...
class debug_port() (implicit val conf: ysyx_24100012_Config)extends BlackBox with HasBlackBoxInline{ 
    val io = IO(new Bundle {
     ...
     import "DPI-C" function void lsu_port( input enable, input fcn,input int addr, input int data);
     ...
     module debug_port(
        ...
        always @(posedge clock) begin
            if (lsu_port_mem_en && lsu_port_fcn == 1'b1) begin
                lsu_port(lsu_port_mem_en,lsu_port_fcn, lsu_port_addr, lsu_port_wdata);
            end else if (lsu_port_valid && lsu_port_fcn == 1'b0) begin
                lsu_port(lsu_port_valid,lsu_port_fcn, lsu_port_addr, lsu_port_rdata);
            end else begin
                lsu_port(1'd0,1'd0,32'd0,32'd0);
            end
        end
     endmodule
     """.stripMargin
