...
class AXI4AR (val addrWidth : Int) (implicit val conf: Config) extends Bundle{
    val addr    =   Output(UInt(conf.xprlen.W))
    val id      =   Output(UInt(conf.idBits.W))
    val len     =   Output(UInt(conf.lenBits.W))  // number of beats - 1
    val size    =   Output(UInt(conf.sizeBits.W)) // bytes in beat = 2^size
...
class AXI4R (val addrWidth : Int) (implicit val conf: Config)extends Bundle{
    val data    =   Output(UInt(addrWidth.W))
    val resp    =   Output(UInt(2.W))
    val id      =   Output(UInt(conf.idBits.W))
    val last    =   Output(Bool())
...
class AXI4AW (val addrWidth : Int) (implicit val conf: Config) extends Bundle{
    val addr    =   Output(UInt(addrWidth.W))
    val id      =   Output(UInt(conf.idBits.W))
    val len     =   Output(UInt(conf.lenBits.W))  // number of beats - 1
    val size    =   Output(UInt(conf.sizeBits.W)) // bytes in beat = 2^size
...
class AXI4W (val addrWidth : Int) (implicit val conf: Config) extends Bundle{
    val data    =   Output(UInt(addrWidth.W))
    val strb    =   Output(UInt(conf.maskBits.W))
    val last    =   Output(Bool())
} 

class AXI4B (val addrWidth : Int) (implicit val conf: Config) extends Bundle{
    val resp    =   Output(UInt(2.W))
    val id      =   Output(UInt(conf.idBits.W))
} 

class AXI4Io (implicit val conf: Config) extends Bundle{ 
    val ar      =   DecoupledIO(new AXI4AR(conf.xprlen))
    val r       =   Flipped(DecoupledIO(new AXI4R(conf.xlen)))
    val aw      =   DecoupledIO(new AXI4AW(conf.xprlen))
    val w       =   DecoupledIO(new AXI4W(conf.xlen))
    val b       =   Flipped(DecoupledIO(new AXI4B(conf.xlen)))
}
...
class AXI4Master (implicit val conf: Config) extends Module{
    val io = IO( new Bundle {
        val clock   =   Input(Clock())
        val reset   =   Input(Bool())
    ...
    val maskWidth   = conf.xlen/8
    val size    = MuxCase(0.U,Seq(
        (io.req.bits.typ(1,0) === 3.U)  ->  2.U,
        (io.req.bits.typ(1,0) === 2.U)  ->  1.U
    ))

    val arvalid = Mux(arfire || rstate===rs_wait_rlast,false.B, is_read)
    val araddr  = Mux(accept_read,io.req.bits.raddr,RegEnable(io.req.bits.raddr,  0.U ,  accept_read||io.axi_io.ar.ready))
    val arlen   = Mux(accept_read, io.req.bits.burstlen,RegEnable(io.req.bits.burstlen,accept_read))
    val arburst = Mux(accept_read,io.req.bits.burst,RegEnable(io.req.bits.burst,accept_read))

    val awaddr  =   Mux(accept_write,io.req.bits.waddr,RegEnable(io.req.bits.waddr, accept_write||io.axi_io.aw.ready))
    val awvalid =   Mux(awfire|| wstate === ws_wait_bvalid,false.B, is_write)
    val wvalid  =   Mux(wfire || wstate === ws_wait_bvalid,false.B, is_write)
    val wlast   =   Mux(wfire || wstate === ws_wait_bvalid,false.B, is_write)
    val wdata   =   Mux(accept_write,io.req.bits.data,RegEnable(io.req.bits.data, 0.U,  accept_write||io.axi_io.w.ready))
    val wstrb   =   Mux(accept_write,io.req.bits.mask,RegEnable(io.req.bits.mask, 0.U,  accept_write||io.axi_io.w.ready))
    val awburst =   io.req.bits.burst

    // val rready = Wire(Bool())
    // val bready = Wire(Bool())

    // rready  := (rstate === rs_wait_rlast) || (rstate === rs_wait_arready ) 
    // bready  := (wstate === ws_wait_bvalid) || (wstate === ws_wait_ready ) 
    
    io.axi_io.ar.bits.addr   := Mux(rstate===rs_idle, io.req.bits.raddr,araddr)
    io.axi_io.ar.valid  := arvalid
    io.axi_io.ar.bits.size   := size
    io.axi_io.ar.bits.id     := 0.U
    io.axi_io.r.ready   := true.B
    io.axi_io.b.ready   := true.B
    
    if(conf.EnableBurst){
        io.axi_io.ar.bits.len    := arlen
        io.axi_io.ar.bits.burst  := arburst
    } else {
        io.axi_io.ar.bits.len    := 0.U
        io.axi_io.ar.bits.burst  := 0.U
    }
    ...
    switch(rstate){
        ...
        is (rs_wait_rlast){ 
            // rlast is high when rvalid is high
            rstate := Mux(io.axi_io.r.bits.last && io.axi_io.r.valid , rs_idle, rs_wait_rlast)
            when (io.axi_io.r.valid){ io.resp.bits.data  := io.axi_io.r.bits.data}
        }
    }
    ...
    //////  AXI4Lite write master
    io.axi_io.aw.valid      :=  Mux(wstate===ws_idle, accept_write,awvalid)
    io.axi_io.aw.bits.addr       :=  awaddr
    io.axi_io.aw.bits.burst      :=  0.U
    io.axi_io.aw.bits.size       :=  size
    io.axi_io.aw.bits.id         :=  0.U
    io.axi_io.aw.bits.len        :=  0.U

    io.axi_io.w.valid       :=  Mux(wstate===ws_idle, accept_write,wvalid)
    io.axi_io.w.bits.data        :=  wdata
    io.axi_io.w.bits.strb        :=  wstrb
    io.axi_io.w.bits.last        :=  wlast
    // write not support burst 
    // io.axi_io.aw.bits.burst      :=  awburst
...
class TopAXI4Slave (implicit val conf: Config) extends Module{
    val io = IO( new Bundle {
        val clock   =   Input(Clock())
        val reset   =   Input(Bool())
        val axi_io  =   Flipped(new AXI4Io())
        val out     =   Flipped(new Bundle{
            val dr      =   new AXIRport(conf.xprlen, conf.xlen)
            val dw      =   new AXIWport(conf.xprlen, conf.xlen)
        })
    ...
    io.axi_io.w.ready       :=   accept_write || (state === s_inflight && is_write)
    io.axi_io.aw.ready      :=   accept_write || (state === s_inflight && is_write)
    val araddr              =   Mux(accept_read,        io.axi_io.ar.bits.addr,         RegEnable(io.axi_io.ar.bits.addr,    io.axi_io.ar.valid))
    val awaddr              =   Mux(accept_write,       io.axi_io.aw.bits.addr,        RegEnable(io.axi_io.aw.bits.addr,    io.axi_io.aw.valid))
    val wdata               =   Mux(io.axi_io.w.valid,  io.axi_io.w .bits.data,    RegEnable(io.axi_io.w .bits.data,     io.axi_io.w.valid))
    val wstrb               =   Mux(io.axi_io.w.valid,  io.axi_io.w .bits.strb,    RegEnable(io.axi_io.w .bits.strb,     io.axi_io.w.valid)) 
